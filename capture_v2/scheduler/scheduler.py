...
class Scheduler:
    def __init__(
        ...
        self.default_classroom_start_time_in_hours_and_minutes = (
            "07:30"
            if default_classroom_start_time_in_hours_and_minutes is None
            else default_classroom_start_time_in_hours_and_minutes
        )
        self.default_classroom_end_time_in_hours_and_minutes = (
            "17:30"
            if default_classroom_end_time_in_hours_and_minutes is None
            else default_classroom_end_time_in_hours_and_minutes
        )
    ...
    def update_tasks(self):
        ...
        environment_start_datetime = datetime.combine(
            ...
            time=datetime.strptime(
                self.default_classroom_start_time_in_hours_and_minutes, "%H:%M"
            ).time(),
            tzinfo=tz,
        )
        environment_end_datetime = datetime.combine(
            date=tz_aware_datetime.date(),
            time=datetime.strptime(
                self.default_classroom_end_time_in_hours_and_minutes, "%H:%M"
            ).time(),
            tzinfo=tz,
        )
