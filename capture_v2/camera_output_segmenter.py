...
class CameraOutputSegmenter(FileOutput):
    def __init__(self,
        ...
        self.monotonic_datetime_start = datetime.fromtimestamp(time.clock_gettime(time.CLOCK_REALTIME) - time.clock_gettime(time.CLOCK_MONOTONIC))
        ...
        while not self.buffer_abort:
            ...
            for filename_key in list(self.segments.keys()):
                ...
                if segment.get('ready', False) == True:
                    ...
                    logger.info(f"Processing '{segment['name']}' - Segment spans '{segment['start_datetime']}' - '{segment['end_datetime']}'")
                    logger.info(f"Processing '{segment['name']}' - Building video file...")
                    df_fitted_frames = None
                    with self.buffer_lock:
                        df_expected_times = pd.date_range(
                                        start=segment['start_datetime'],
                                        end=segment['end_datetime'] - timedelta(milliseconds=1000/self.frame_rate),
                                        freq=f"{1000/self.frame_rate}L").to_frame(index=False, name='frame_time')
                        ...
                        self.frame_buffer[:] = filterfalse(lambda item: item['image_timestamp'] < invalidate_timestamp, self.frame_buffer)
                    ...
                    with pts_file_path.open(mode="w+", encoding="utf-8") as pts_file:
                        for idx, frame in df_fitted_frames.iterrows():
                            relative_pts = (frame['image_timestamp'] - segment['start_datetime']).total_seconds()
                            if idx == 0:
                                pts_file.write("# timestamp format v2\n0\n")
                            else:
                                pts_file.write(f"{(relative_pts * 1000):.3f}\n")

                            output.outputframe(frame=frame['data'], keyframe=frame['keyframe'], timestamp=relative_pts * 1e9)

                    output.stop()

                    logger.info(f"Processing '{segment['name']}' - Video created! Total frames: {len(df_fitted_frames)}")
                    
                    cmds = []
                    if isinstance(output, FileOutput):
                        logger.info(f"Processing '{segment['name']}' - Converting mjpeg to mp4...")
                        cmds.append(f"ffmpeg -f mjpeg -r {self.frame_rate} -loglevel warning -y -thread_queue_size 32 -i {segment['source_filepath']} -pix_fmt yuv420p -b:v 4M -c:v h264_v4l2m2m -f mp4 {segment['mp4_filepath']}")

                    cmds.append(f"sleep 1") # The mp4 file isn't quite ready it seems...
                    cmds.append(f"mv {segment['mp4_filepath']} {segment['mp4_filepath']}.tmp")
                    # Next step is to update the PTS timestamps. We use mp4fpsmod
                    cmds.append(f"mp4fpsmod -t {segment['pts_filepath']} {segment['mp4_filepath']}.tmp -o {segment['mp4_filepath']}")
                    ...
                    cmds.append(f"rm -f {segment['pts_filepath']} {segment['mp4_filepath']}.tmp {segment['source_filepath']}")
                    ...
                    popen_and_call(thread_safe_callback(done_msg), {"args": '; '.join(cmds.copy()), "shell": True})
    ...
    def rotate(self):
        ...
        self.clip_end_datetime = self.clip_end_datetime + timedelta(seconds=self.clip_duration)
        self.current_clip_frame_count = 0
    ...
    def outputframe(self, frame, keyframe=True, timestamp=None):
        loose_start_datetime = self.start_datetime - timedelta(milliseconds=1000/self.frame_rate)
        loose_end_datetime = None
        if self.end_datetime:
            loose_end_datetime = self.end_datetime + timedelta(milliseconds=1000/self.frame_rate)

        loose_clip_end_datetime = self.clip_end_datetime + timedelta(milliseconds=1000/self.frame_rate)
        ...
        image_timestamp = self.monotonic_datetime_start + timedelta(seconds=self.camera_monotonic_start_time_in_seconds + frame_timestamp_in_seconds_from_init)
        ...
        valid_time = (
            image_timestamp >= loose_start_datetime and
            (loose_end_datetime is None or (loose_end_datetime is not None and image_timestamp < loose_end_datetime))
        )
        if valid_time:
            ...
            with self.buffer_lock:
                self.frame_buffer.append({"image_timestamp": image_timestamp, "timestamp": timestamp, "keyframe": keyframe, "data": frame})
