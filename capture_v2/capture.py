...
video_config = picam2.create_video_configuration(
    main={"size": (1296, 972), "format": "YUV420"}, lores={"size": (640, 360)}
)
...
monotonic_datetime_baseline = datetime.fromtimestamp(
    time.clock_gettime(time.CLOCK_REALTIME) - time.clock_gettime(time.CLOCK_MONOTONIC)
)
...
try:
    ...
    if not STREAM_ONLY:
        ...
        capture_start_in_monotonic_seconds = (
            picam2.capture_metadata()["SensorTimestamp"] / 1e9
        )
        custom_output.set_camera_monotonic_start_time(
            capture_start_in_monotonic_seconds
        )
        logger.info(
            f"PiCam start in system monotonic seconds: {capture_start_in_monotonic_seconds}"
        )
        logger.info(
            f"PiCam start in datetime: {(monotonic_datetime_baseline + timedelta(seconds=capture_start_in_monotonic_seconds)).isoformat()}"
        )
    else:
        # picam2.start()
    ...
    mjpeg_stream_thread = threading.Thread(
        target=start_mjpeg_stream, args=(stop_threads_event,)
    )
    ...
    logger.info(
        f"System's monotonic baseline in datetime: {monotonic_datetime_baseline.isoformat()}"
    )
